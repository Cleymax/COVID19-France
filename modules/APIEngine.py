...
def get_data(source):
    if source == "WORLDOMETERS":
        ...
        for i in range(len(gouv_data)):
            if gouv_data[i]['sourceType'] == "ministere-sante":
                if gouv_data[i]['date'] == dateSelected:
                    try:
                    ...
                    try:
                        if int(get_config("CustomData", "casMalades")) > 0:
                            casMalades = int(get_config("CustomData", "casMalades"))
                            print("[ATTENTION] Chiffres 'casMalades' modifiés manuellements")
                    except:
                        casMalades = cas_confirmes - (total_deces + cas_gueris)
